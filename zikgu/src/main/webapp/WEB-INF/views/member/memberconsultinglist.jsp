		  ...
		  <sec:authentication property="principal" var="principal"/>
       	<form action="consulting_search_All?u_key=${principal.u_key}" method="post" style="width:1200px;" >
		          <input class="form-control form-control-dark  rounded-0 border-0"  aria-label="Search" type="text" name="keyword" placeholder="트레이너명 검색" value="${keyword }">
		              ...
		              <sec:authentication property="principal" var="principal"/>
		          <a href="consulting_search_All?keyword=${keyword}&u_key=${principal.u_key}" class="search_icon" ><i class="fas fa-search"></i></a>
          	</form>
...
function allbutton_click() {
	console.log("전체버튼을 누르셨습니다.");
	  var u_key =$('#u_key').val(); 	 
	  var url = "/memberconsulting?u_key=" + encodeURIComponent(u_key);  
	  window.location.href = url;
}
function completebutton_click() {
	console.log("완료버튼을 누르셨습니다.");
	 var u_key =$('#u_key').val(); 	 
	  var url = "/memberconsultingcomplete?u_key=" + encodeURIComponent(u_key);  
	  window.location.href = url;
}
function waitebutton_click() {
	console.log("대기버튼을 누르셨습니다.");
	 var u_key =$('#u_key').val(); 	 
	  var url = "/memberconsultingwaite?u_key=" + encodeURIComponent(u_key);  
	  window.location.href = url;
}
...
function search_allbutton_click() {
	...
	var u_key = $('#u_key').val();
	var path = '/consulting_search_All';
...
function search_completebutton_click() {
	...
	var keyword = $('#keyword').val();
	var u_key =$('#u_key').val();
	var path = '/consulting_search_complete';
	var url = path + '?keyword=' + encodeURIComponent(keyword) + '&u_key=' + encodeURIComponent(u_key);
	location.href = url;
}
function search_waitebutton_click() {
	...
	var keyword = $('#keyword').val();
	var u_key =$('#u_key').val();
	var path = '/consulting_search_waite';
	var url = path + '?keyword=' + encodeURIComponent(keyword) + '&u_key=' + encodeURIComponent(u_key);
	location.href = url;
}
</script>
